...
const WaitingView = ({ teamMembers, myId, teamId }: { teamMembers: TeamMember[], myId: number, teamId: number }) => {
  ...
  const handleReady = async () => {
    try {
      await authenticatedFetch(`/api/v1/teams/${teamId}/ready`, {
        method: 'POST',
      ...
      alert('준비 상태를 업데이트하는 데 실패했습니다.');
    }
  };

  const handleLeaveTeam = () => {
    alert('팀 나가기 기능은 현재 구현되지 않았습니다. 이전 페이지로 이동합니다.');
    router.back();
  };
      ...
      <main className="relative z-20 w-full max-w-md mx-auto transform -translate-y-16">
        ...
        {me && !me.is_ready && (
          <div className="mt-2 text-center">
            <Button onClick={handleReady} className="w-full">준비 완료</Button>
          </div>
        )}
...
export default function Page2() {
  ...
  useEffect(() => {
    ...
    const checkUserTeam = async () => {
      ...
      try {
        const response = await authenticatedFetch(`/api/v1/users/${myId}/team`);
        if (response.ok) {
